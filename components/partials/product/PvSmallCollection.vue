<template>
  <div class="product-widgets-container row pb-2">
    ...
    <div class="col-lg-3 col-sm-6 pb-5 pb-md-0">
      <h4 class="section-sub-title">{{ $t("Best Selling Products") }}</h4>

      <div v-if="!bestProducts || bestProducts.length === 0">
        <div
          class="skel-product-col skel-pro mb-2"
          v-for="(item, index) in [0, 1, 2]"
          :key="'best' + index"
    ...
    <div class="col-lg-3 col-sm-6 pb-5 pb-md-0">
      <h4 class="section-sub-title">{{ $t("Latest Products") }}</h4>

      <div v-if="!latestProducts || latestProducts.length === 0">
        <div
          class="skel-product-col skel-pro mb-2"
          v-for="(item, index) in [0, 1, 2]"
          :key="'latest' + index"
    ...
    <div class="col-lg-3 col-sm-6 pb-5 pb-md-0">
      <h4 class="section-sub-title">{{ $t("Top Rated Products") }}</h4>

      <div v-if="!topRatedProducts || topRatedProducts.length === 0">
        <div
          class="skel-product-col skel-pro mb-2"
          v-for="(item, index) in [0, 1, 2]"
          :key="'top' + index"
