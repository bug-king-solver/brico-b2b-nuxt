...
export function getProductsByAttri(products, attri = "is_hot") {
  // Temp functionality for static view
  let temp = [];
  if (products.length === 1) {
    temp = new Array(6).fill(products[0]);
  } else {
    temp = [...products];
  }
  //   end Temp functionality for static view
  //   origin : let temp = [...products];
  ...
  return temp.filter((item) => {
    ...
    if (Array.isArray(category)) {
      for (let i = 0; i < category.length; i++) {
        if (
          -1 !==
          item.product_categories.findIndex((item) => item.name === category[i])
        ) {
          result = true;
        }
      }
      ...
      if (
        -1 !==
        item.product_categories.findIndex((item) => item.name === category)
      )
        result = true;
    }
    return result;
